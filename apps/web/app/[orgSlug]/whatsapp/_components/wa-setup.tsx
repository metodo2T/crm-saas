    ...
    <div className="max-w-lg mx-auto mt-12 pb-12">
      ...
      <div className="space-y-3 mb-6">
        ...
        <div className="flex gap-3 bg-white border border-slate-200 rounded-xl p-4">
          <div className="w-6 h-6 rounded-full bg-green-600 text-white text-xs font-bold flex items-center justify-center shrink-0 mt-0.5">1</div>
          <div>
            <p className="text-sm font-semibold text-slate-800">Crie uma conta na Z-API</p>
            <p className="text-xs text-slate-500 mt-0.5">
              Acesse{' '}
              <a href="https://app.z-api.io" target="_blank" rel="noreferrer" className="text-blue-600 underline font-medium">
                app.z-api.io
              </a>{' '}
              → crie conta gratuita → crie uma instância → copie o <strong>Instance ID</strong> e o <strong>Token</strong>.
            </p>
          </div>
        </div>
        ...
        {/* Step 2 */}
        <div className="flex gap-3 bg-white border border-slate-200 rounded-xl p-4">
          <div className="w-6 h-6 rounded-full bg-green-600 text-white text-xs font-bold flex items-center justify-center shrink-0 mt-0.5">2</div>
          <div className="flex-1 min-w-0">
            <p className="text-sm font-semibold text-slate-800">Configure o Webhook na Z-API</p>
            <p className="text-xs text-slate-500 mt-0.5 mb-2">
              Na sua instância Z-API → <strong>Webhooks</strong> → cole a URL abaixo no campo de webhook de mensagens recebidas:
            </p>
            <div className="flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-lg px-3 py-2">
              <code className="text-xs text-slate-700 flex-1 truncate">{webhookUrl}</code>
              <button
                onClick={copyWebhook}
                className="text-xs px-2 py-1 rounded bg-green-600 text-white hover:bg-green-700 shrink-0 transition-colors"
              >
        ...
        {/* Step 3 */}
        <div className="flex gap-3 bg-white border border-slate-200 rounded-xl p-4">
          <div className="w-6 h-6 rounded-full bg-green-600 text-white text-xs font-bold flex items-center justify-center shrink-0 mt-0.5">3</div>
          <div className="flex-1">
            <p className="text-sm font-semibold text-slate-800">Cole as credenciais e salve</p>
            <div className="mt-3 space-y-3">
              ...
              <div>
                <label className="block text-xs font-medium text-slate-600 mb-1">Instance Token</label>
                <input
                  type="password"
        ...
        {(showQr || isConfigured) && instance?.status !== 'CONNECTED' && (
          <div className="flex gap-3 bg-white border border-slate-200 rounded-xl p-4">
            <div className="w-6 h-6 rounded-full bg-green-600 text-white text-xs font-bold flex items-center justify-center shrink-0 mt-0.5">4</div>
            <div className="flex-1">
              <p className="text-sm font-semibold text-slate-800">Escaneie o QR Code</p>
              <p className="text-xs text-slate-500 mt-0.5 mb-3">
                No WhatsApp do seu celular: <strong>Menu (⋮) → Aparelhos conectados → Conectar um aparelho</strong>
              </p>
              {!showQr ? (
                <Button size="sm" variant="outline" onClick={() => setShowQr(true)} className="text-xs">
                  Ver QR Code
                </Button>
              ) : qrLoading ? (
                ...
                <div className="flex flex-col items-center gap-2 max-w-[220px]">
                  ...
                  <img src={qr.base64} alt="QR Code WhatsApp" className="w-52 h-52 rounded-xl border border-slate-200" />
                  <p className="text-xs text-slate-400">Atualiza automaticamente a cada 20s</p>
                </div>
              ) : (
